...
struct HomogeneousPolynomial
{
    ...
    Scalar_ evaluate (Vector const &at) const
    {
        ...
        return m_coefficients(i)*OuterPowerOf(at)(i);
    }
    ...
    HomogeneousPolynomial operator* (Scalar_ const &rhs) const
    {
        ...
        term(i) = rhs*m_coefficients(i);
        return HomogeneousPolynomial(term);
    }
    ...
    {
        Tenh::AbstractIndex_c<'i'> i;
        SymDual result(Tenh::Static<Tenh::WithoutInitialization>::SINGLETON);
        result(i) = m_coefficients(i) + rhs.m_coefficients(i);
    ...
    HomogeneousPolynomial<Other_Degree + DEGREE_,DIMENSION_,Id_,Scalar_> operator* (HomogeneousPolynomial<Other_Degree,DIMENSION_,Id_,Scalar_> const &rhs) const
    {
        ...
        ResultSymDual result(Tenh::Static<Tenh::WithoutInitialization>::SINGLETON);
        ...
        result(i) = (m_coefficients(j).split(j,J)*rhs.m_coefficients(k).split(k,K)).bundle(J|K,ResultingTensorPowerType(),I)*symmetrize(i|I);
        ...
        for (ResultComponentIndex it; it.is_not_at_end(); ++it)
        {
            ...
            result[it] *= static_cast<Scalar_>(Tenh::MultiIndexMultiplicity_t<ResultMultiIndex>::eval(m))
                          / static_cast<Scalar_>(Tenh::Factorial_t<Other_Degree + DEGREE_>::V);
        }

        return ResultPolynomial(result);
    }
...
// struct HomogeneousPolynomial<0,DIMENSION_,Id_,Scalar_>
// {
...
//
//     HomogeneousPolynomial (Scalar_ const &leading_term) : m_coefficients(leading_term) { }
//     HomogeneousPolynomial (Tenh::WithoutInitialization const &w) { }
//     HomogeneousPolynomial (HomogeneousPolynomial const &other) : m_coefficients(other.m_coefficients) { }
//
//     Scalar_ evaluate (Vector const &at) const { return m_coefficients; }
//
//     // opertor overloads
//     HomogeneousPolynomial operator+ (HomogeneousPolynomial const &rhs) const { return HomogeneousPolynomial(m_coefficients + rhs.m_coefficients)); }
//
//     HomogeneousPolynomial operator+ (Scalar_ const &rhs) const { return HomogeneousPolynomial(rhs+m_coefficients); }
//
//
//     HomogeneousPolynomial operator- () const { return HomogeneousPolynomial(Scalar_(-1)*m_coefficients); }
//
//     HomogeneousPolynomial operator- (HomogeneousPolynomial const &rhs) const { return HomogeneousPolynomial(m_coefficients - rhs.m_coefficients)); }
//
//
...
//
//     template <Uint32 Other_Degree>
//     HomogeneousPolynomial<Other_Degree,DIMENSION_,Id_,Scalar_> operator* (HomogeneousPolynomial<Other_Degree,DIMENSION_,Id_,Scalar_> const &rhs) const
//     {
//         typedef HomogeneousPolynomial<Other_Degree,DIMENSION_,Id_,Scalar_> ResultPolyType;
//         Tenh::AbstractIndex_c<'i'> i;
//
//         typename ResultPolyType::SymDual result;
//
//         result(i) = m_coefficients * rhs.m_coefficients(i)
//
//         return ResultPolyType(result);
//     }
//
//     HomogeneousPolynomial operator* (Scalar_ const &rhs) const { return HomogeneousPolynomial(rhs*m_coefficients); }
//
...
//
// private:
//     Scalar_ m_coefficients;
//
//     template<Uint32,Uint32,typename,typename>
//     friend struct HomogeneousPolynomial;
//     template<Uint32 DIM, typename Id, typename Scalar>
//     friend std::ostream &operator << (std::ostream &out, HomogeneousPolynomial<0,DIM,Id,Scalar> const &m);
// };
