                                                                   ...
                                                                   Derived_>::T>
{
    ...
    Scalar const &operator [] (CompoundIndex_t<TypeList_t<Index1,TypeList_t<Index2> > > const &c) const
    {
        ...
        // strict upper-diagonal elements are stored contiguously in row-major order, starting at element D := DIAGONAL_COMPONENT_COUNT.
    ...
    // there is redundant access to components here, since the subdiagonal components are
    // really just aliases for the superdiagonal components.
    template <typename Index1, typename Index2>
