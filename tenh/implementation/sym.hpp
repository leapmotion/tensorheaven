...
Scalar_ sym (ComponentIndex_t<COMPONENT_COUNT_> const &i)
{
    ...
    typedef ImplementationOf_t<SymmetricPower,Scalar_,UsePreallocatedArray_t<COMPONENTS_ARE_NONCONST> > Factor0BootstrappingImplementation;
...
struct SymId_t
{
    static std::string type_as_string () { return "Sym<" + AS_STRING(ORDER_) + ',' + type_string_of<Factor_>() + '>'; }
};
