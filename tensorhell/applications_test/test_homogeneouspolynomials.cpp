...
{
    Directory *vector = new Directory(FORMAT("HomogeneousPolynomial<" << DEGREE << ',' << DIM << ',' << Tenh::type_string_of<PolynomialBasisId>() << ',' << Tenh::type_string_of<Scalar>() << '>'), parent);
...
{
    Directory *vector = new Directory("HomogeneousPolynomial", parent);
    // add_particular_tests_for_scalar<Tenh::Sint8>(vector);
    // add_particular_tests_for_scalar<Tenh::Sint32>(vector);
